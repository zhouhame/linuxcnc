...
static void printStatus()
{
  ...
  if (window == helpwin)
  ...
  else if (window == diagwin)
    {
      ...
      sprintf(scratch_string, "%10.3f", emcStatus->motion.joint[0].scale);
      mvwaddstr(window, 13, 28, scratch_string);
      sprintf(scratch_string, "%10.3f", emcStatus->motion.joint[1].scale);
      mvwaddstr(window, 14, 28, scratch_string);
      sprintf(scratch_string, "%10.3f", emcStatus->motion.joint[2].scale);

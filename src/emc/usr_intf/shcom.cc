...
{
    EMC_JOINT_ABORT emc_axis_abort_msg;
...
int sendAxisSetBacklash(int joint, double backlash)
{
...
int sendAxisSetOutput(int joint, double output)
{
    EMC_JOINT_SET_OUTPUT emc_joint_set_output_msg;

    emc_joint_set_output_msg.joint = joint;
    emc_joint_set_output_msg.output = output;
    emc_joint_set_output_msg.serial_number = ++emcCommandSerialNumber;
    emcCommandBuffer->write(emc_joint_set_output_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
	return emcCommandWaitReceived(emcCommandSerialNumber);
    } else if (emcWaitType == EMC_WAIT_DONE) {
	return emcCommandWaitDone(emcCommandSerialNumber);
...
int sendAxisEnable(int joint, int val)
{
...
int sendAxisLoadComp(int joint, const char *file, int type)
{

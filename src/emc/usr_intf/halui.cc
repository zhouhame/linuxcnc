...
static int sendJogStop(int joint)
{
    ...
    if ( ((emcStatus->motion.traj.mode != EMC_TRAJ_MODE_TELEOP) && (joint < 0 || joint >= EMC_JOINT_MAX)) ||
	 ((emcStatus->motion.traj.mode == EMC_TRAJ_MODE_TELEOP) && (joint < 0 || joint >= EMC_AXIS_MAX))) {
	return -1;
    }

    emc_jog_stop_msg.serial_number = ++emcCommandSerialNumber;
    emc_jog_stop_msg.axis = joint;
...
static int sendJogCont(int axis, double speed)
{
    ...
    if ( ((emcStatus->motion.traj.mode != EMC_TRAJ_MODE_TELEOP) && (axis < 0 || axis >= EMC_JOINT_MAX)) ||
	 ((emcStatus->motion.traj.mode == EMC_TRAJ_MODE_TELEOP) && (axis < 0 || axis >= EMC_AXIS_MAX))) {
	return -1;
    }

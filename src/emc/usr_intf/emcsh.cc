...
static int emc_joint_set_output(ClientData clientdata,
			       Tcl_Interp * interp, int objc,
			       Tcl_Obj * CONST objv[])
{
    int joint;
    ...
    // syntax is emc_joint_output <joint> <output>

    if (objc != 3) {
	Tcl_SetResult(interp, "emc_joint_set_output: need <joint> <output>",
		      TCL_VOLATILE);
	return TCL_ERROR;
    }
	...
	joint < 0 || joint >= EMC_JOINT_MAX) {
	Tcl_SetResult(interp,
		      "emc_joint_set_output: need joint as integer, 0..EMC_JOINT_MAX-1",
		      TCL_VOLATILE);
	return TCL_ERROR;
    }
	...
	Tcl_SetResult(interp,
		      "emc_joint_set_output: need output as real number",
		      TCL_VOLATILE);
	return TCL_ERROR;
    }
    // now write it out
    sendAxisSetOutput(joint, output);
    return TCL_OK;
}
...
int Tcl_AppInit(Tcl_Interp * interp)
{
    ...
    Tcl_CreateObjCommand(interp, "emc_joint_set_output",
			 emc_joint_set_output, (ClientData) NULL,
			 (Tcl_CmdDeleteProc *) NULL);

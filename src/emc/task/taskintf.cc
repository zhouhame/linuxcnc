...
#ifdef ISNAN_TRAP
    if (isnan(emcmotCommand.maxLimit) || isnan(emcmotCommand.minLimit)) {
	printf("isnan error in emcJointSetMaxPosition\n");
	return -1;
    }
#endif
...
int emcJointSetHomingParams(int joint, double home, double offset, double home_vel,
			   ...
			   int sequence,int volatile_home)
{
    ...
    emcmotCommand.home_vel = home_vel;
	...
	printf("isnan error in emcJointSetHoming\n");
	return -1;
    }
#endif
...
/*! \todo FIXME - should all homing config params be included here? */
	...
	stat[joint_num].setpoint = joint->pos_cmd;
	/*! \todo FIXME - output is the DAC output, now part of HAL */
	stat[joint_num].output = 0.0;
